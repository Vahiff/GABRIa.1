...
function Metric({ label, value, unit, icon: Icon }) {
  return (
    <section className="card metric">
      <div className="metricTop"><Icon size={18}/><span>{label}</span></div>
      <strong>{value}</strong>
      <small>{unit}</small>
    </section>
  );
  ...
  useEffect(() => {
    ...
    const check = async () => {
      try {
        ...
        if (alive) setStatus(j?.ok ? 'backend online' : 'demo mode');
      } catch {
        if (alive) setStatus('demo mode');
      }
    };
    check();
  ...
  return (
    <main className="shell">
      ...
      <header className="topbar">
        <div className="brand">
          <div className="logo"><Car size={24}/></div>
          <div><b>Fleet Prestige</b><span>Premium Audi Diagnostics</span></div>
        </div>
